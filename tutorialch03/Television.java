...
public class Television {
	...
	/*
	 * Connect UsbDevice using USB object.
	 */
	public boolean connectUSBDevice(USBDevice usbDevice) {
		if(!isUSBConnected()) {
			fileListIndex = 0;
			usb = new USB(usbDevice, this);
			return true;
		} else {
			return false;
		}
	...
	public ArrayList<String> getFileListInUSB() {
		return usb.getUSBDevice().getFileList();
	}
	...
	public String getFileInUSB(int index) {
		if(usb != null && on) {
			ArrayList<String> fileList = usb.getUSBDevice().getFileList();
			if(index < 0) {
				index = 0;
			}
	...
	public void disconnectUSBDevice() {
		usb.getUSBDevice().disconnectDevice();
		usb = null;
		fileListIndex = 0;
	}
